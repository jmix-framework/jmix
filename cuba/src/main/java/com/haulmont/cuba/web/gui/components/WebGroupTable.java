...
@Deprecated
public class WebGroupTable<E extends Entity> extends GroupTableImpl<E>
        implements GroupTable<E>, LookupComponent.LookupSelectionChangeNotifier<E>,
        ColumnManager, GroupColumnManager {
    ...
    public void setColumnGroupAllowed(String columnId, boolean allowed) {
        Table.Column column = getColumnNN(columnId);

        if (column instanceof GroupColumn) {
            setColumnGroupAllowed((GroupColumn) column, allowed);
        }
    }
    ...
    public void setColumnGroupAllowed(Table.Column column, boolean allowed) {
        ...
        if (column instanceof GroupColumn) {
            ((GroupColumn<?>) column).setGroupAllowed(allowed);
        } else if (column instanceof com.haulmont.cuba.gui.components.Table.Column) {
            ((com.haulmont.cuba.gui.components.Table.Column<?>) column).setGroupAllowed(allowed);
        }
    }
        ...
        for (MetaPropertyPath metaPropertyPath : paths) {
            ...
            if (!property.getRange().getCardinality().isMany()
                ...
                if (column instanceof com.haulmont.cuba.gui.components.Table.Column) {
                    ((com.haulmont.cuba.gui.components.Table.Column<E>) column).setType(metaPropertyPath.getRangeJavaClass());
                }
            }
        }
    }
