...
public class WebTable<E extends Entity> extends TableImpl<E>
        implements Table<E>, LookupComponent.LookupSelectionChangeNotifier<E>, ColumnManager {
    ...
    @Override
    protected Consumer<io.jmix.ui.component.Table.Column.ClickEvent> createLinkCellClickListener() {
    ...
    protected void onCellClick(JmixEnhancedTable.TableCellClickEvent event) {
        ...
        if (!(getColumn(String.valueOf(event.getColumnId())) instanceof Table.Column)) {
            return;
        }

        Table.Column<E> column = (Table.Column<E>) getColumn(String.valueOf(event.getColumnId()));
