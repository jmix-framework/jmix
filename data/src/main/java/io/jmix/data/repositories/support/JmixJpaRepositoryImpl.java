...
import static io.jmix.data.repositories.query.utils.LoaderHelper.springToJmixSort;

@NoRepositoryBean
public class JmixJpaRepositoryImpl<T, ID extends Serializable> implements JmixJpaRepository<T, ID> {
    ...
    private String defaultFetchPlanName = FetchPlan.LOCAL;

    private Class<T> domainClass;

    public JmixJpaRepositoryImpl(Class<T> domainClass, DataManager dataManager, Metadata metadata) {
    ...
    @Override
    public Iterable<T> findAll(Iterable<ID> ids, String fetchPlan) {
    ...
    public Optional<T> findById(ID id) {
        return Optional.of(dataManager.load(domainClass).id(id).fetchPlan(defaultFetchPlanName).one());
    }
    ...
    public Iterable<T> findAll() {
        return dataManager.load(domainClass).all().fetchPlan(defaultFetchPlanName).list();
    }

    @Override
    public Iterable<T> findAllById(Iterable<ID> ids) {
        return findAll(ids, defaultFetchPlanName);
    }
    ...
    @Override
    public String getDefaultFetchPlanName() {
        return defaultFetchPlanName;
    }

    @Override
    public void setDefaultFetchPlanName(String defaultFetchPlanName) {
        this.defaultFetchPlanName = defaultFetchPlanName;
    }
    ...
    public Iterable<T> findAll(Sort sort) {
        return dataManager.load(domainClass).all().fetchPlan(defaultFetchPlanName).sort(springToJmixSort(sort)).list();
    }
    ...
    public Page<T> findAll(Pageable pageable) {
        FluentLoader.ByCondition<T> loader = dataManager.load(domainClass)
                .all()
                .fetchPlan(defaultFetchPlanName);

        LoaderHelper.applyPageableForConditionQuery(loader, pageable);
