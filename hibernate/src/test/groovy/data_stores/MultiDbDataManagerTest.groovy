...
class MultiDbDataManagerTest extends DataSpec {
    ...
    void testOneStore() {
        ...
        storeAwareLocator.getTransactionTemplate("db1").executeWithoutResult {
            ...
            Db1Customer c = em.find(Db1Customer.class, customer.getId())
            assert c == customer
        }

        and:
        Db1Customer c = dataManager.load(Db1Customer.class).id(customer.getId()).one()
        customer == c
    ...
    void testTwoStores() {
        ...
        storeAwareLocator.getTransactionTemplate("db1").executeWithoutResult {
            EntityManager em = storeAwareLocator.getEntityManager("db1")
            Db1Customer customer1 = em.find(Db1Customer.class, customer.id)
