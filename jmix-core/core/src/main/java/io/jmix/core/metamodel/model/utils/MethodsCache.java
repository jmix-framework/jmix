    ...
    private MethodsCache(Class clazz) {
        ...
        for (Method method : methods) {
            ...
            if (name.startsWith("get") && method.getParameterTypes().length == 0) {
                ...
                Field justField = ReflectionUtils.findField(clazz, StringUtils.uncapitalize(name.substring(3)));
                if (isField == null || justField != null) {//property name is not the same as getter name
                ...
                Field justField = ReflectionUtils.findField(clazz, StringUtils.uncapitalize(name.substring(3)));//must prefer exact field instead of kotlin-style field getter
                if (isField != null && justField == null) {
