...
public class FileStorageException extends Exception {
    ...
    public FileStorageException(io.jmix.core.FileStorageException runtimeException) {
        super(runtimeException.getMessage() + ": " + runtimeException.getFileName(), runtimeException.getCause());
        switch (runtimeException.getType()) {
            case IO_EXCEPTION:
                this.type = Type.IO_EXCEPTION;
                break;
            case FILE_ALREADY_EXISTS:
                this.type = Type.FILE_ALREADY_EXISTS;
                break;
            case FILE_NOT_FOUND:
                this.type = Type.FILE_NOT_FOUND;
                break;
            case MORE_THAN_ONE_FILE:
                this.type = Type.MORE_THAN_ONE_FILE;
                break;
            case STORAGE_INACCESSIBLE:
                this.type = Type.STORAGE_INACCESSIBLE;
                break;
        }
        this.fileName = runtimeException.getFileName();
    }
