...
@Component("h2SequenceSupport")
public class H2SequenceSupport implements SequenceSupport {

    @Override
    public String sequenceExistsSql(String sequenceName) {
        return "select top 1 SEQUENCE_NAME from INFORMATION_SCHEMA.SEQUENCES where SEQUENCE_NAME = '"
    ...
    public String getCurrentValueSql(String sequenceName) {
        return "select (cast(BASE_VALUE as bigint) - 1) from INFORMATION_SCHEMA.SEQUENCES where SEQUENCE_NAME = '"
                + sequenceName.toUpperCase() + "'";
    }
}
