...
public class EntityRestoreImpl implements EntityRestore {
    ...
    protected Object addOffsetToDateMs(Object dateObject, int offset) {
        if (dateObject instanceof Date) {
            return DateUtils.addMilliseconds((Date) dateObject, offset);
        } else if (dateObject instanceof LocalDate) {
            return dateObject;
        } else if (dateObject instanceof LocalDateTime) {
            return ((LocalDateTime) dateObject).plus(offset, ChronoUnit.MILLIS);
        } else if (dateObject instanceof OffsetDateTime) {
            return ((OffsetDateTime) dateObject).plus(offset, ChronoUnit.MILLIS);
        } else {
            throw new IllegalArgumentException(String.format("'%s' date class is not supported",
                    dateObject.getClass().getSimpleName()));
        }
    }
