...
import static io.jmix.flowui.devserver.frontend.FrontendUtils.WEB_COMPONENT_HTML;
...
public final class ViteHandler extends AbstractDevServerRunner {
     ...
     */
    public ViteHandler(Lookup lookup, int runningPort, File npmFolder, File studioFolder, CompletableFuture<Void> waitFor) {
    ...
    protected File getServerBinary() {
        return new File(getStudioRoot(), VITE_SERVER);
    }

    @Override
    protected File getServerConfig() {
        return new File(getStudioRoot(), FrontendUtils.VITE_CONFIG);
    }
