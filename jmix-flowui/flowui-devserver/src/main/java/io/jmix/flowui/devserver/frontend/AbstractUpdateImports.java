...
abstract class AbstractUpdateImports implements Runnable {
    ...
    private static final Pattern FRONTEND_IMPORT_LINE = Pattern.compile(
            String.format(IMPORT_TEMPLATE, FrontendUtils.WEBPACK_PREFIX_ALIAS + "\\S*"));
    ...
    protected String resolveResource(String importPath) {
        ...
        if (!importPath.startsWith("@")) {
            ...
            if (hasMetaInfResource(resource)) {
                ...
                resolved = FrontendUtils.FLOW_NPM_PACKAGE_NAME + resource;
            }
        }
        return resolved;
    }
    ...
    private File getImportedFrontendFile(String jsImport) {
        ...
        file = getFile(getNodeModulesDir(), FrontendUtils.FLOW_NPM_PACKAGE_NAME, jsImport);
        return file.exists() ? file : null;
    }

    private File getNodeModulesDir() {
        return new File(npmDir, FrontendUtils.NODE_MODULES);
    }
    ...
    private String toValidBrowserImport(String jsImport) {
        if (jsImport.startsWith(com.vaadin.flow.server.frontend.NodeUpdater.GENERATED_PREFIX)) {
            ...
            return FrontendUtils.WEBPACK_PREFIX_ALIAS + jsImport.replaceFirst("^\\./", "");
        }
        return jsImport;
    }
