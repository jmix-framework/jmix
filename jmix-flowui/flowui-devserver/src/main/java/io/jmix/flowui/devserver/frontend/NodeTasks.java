...
//                }
        ...
        commands.add(new TaskGenerateBootstrap(frontendDependencies, options, themeDefinition));
        ...
        if (themeDefinition != null) {
            themeName = themeDefinition.getName();
        }

        PwaConfiguration pwa;
        if (frontendDependencies != null) {
            if (themeName.isBlank()) {
                ThemeDefinition definition = frontendDependencies.getThemeDefinition();
                themeName = definition.getName();
            }
    ...
    private ThemeDefinition createThemeDefinition(Options options, ClassFinder classFinder) {
        if (options.getThemeClass() == null) {
            return null;
        }

        Class<? extends AbstractTheme> themeClass = getThemeClass(classFinder, options.getThemeClass());
        return new ThemeDefinition(themeClass, options.getThemeVariant(), options.getThemeValue());
    }
