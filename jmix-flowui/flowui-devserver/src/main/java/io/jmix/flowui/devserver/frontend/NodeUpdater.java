...
public abstract class NodeUpdater implements FallibleCommand {
    ...
    protected final File studioFolder;
    ...
    protected File getStudioJsonFile() {
        return new File(studioFolder, PACKAGE_JSON);
    }
    ...
    protected File getStudioPackageLockFile() {
        return new File(studioFolder, PACKAGE_LOCK_JSON);
    }
     ...
     * @throws IOException when versions file could not be read
     */
    ...
    JsonObject getPackageJson(File packageFile) throws IOException {
        JsonObject packageJson = getJsonFileContent(packageFile);
        if (packageJson == null) {
    ...
    private void removeWebpackPlugins(JsonObject packageJson) {
        ...
        String pluginTargetPrefix = "./"
                + (studioFolder.toPath().relativize(targetFolder) + "/")
                .replace('\\', '/');
    ...
    protected String generateVersionsJson() throws IOException {
        ...
        if (versions.isAbsolute()) {
            return FrontendUtils.getUnixRelativePath(studioFolder.toPath(), versionsPath);
        } else {
    ...
    private JsonObject generateVersionsFromPackageJson() throws IOException {
        ...
        final JsonObject packageJson = getPackageJson(getStudioJsonFile());
