...
public class TaskGenerateServiceWorker extends AbstractTaskClientGenerator {
    ...
    protected String getFileContent() throws IOException {
        try (InputStream swStream = FrontendUtils.getResourceAsStream(
                FrontendUtils.SERVICE_WORKER_SRC, TaskGenerateServiceWorker.class.getClassLoader()
        )) {
    ...
    protected File getGeneratedFile() {
        return new File(outputDirectory, FrontendUtils.SERVICE_WORKER_SRC);
    }

    @Override
    protected boolean shouldGenerate() {
        File serviceWorker = new File(frontendDirectory, FrontendUtils.SERVICE_WORKER_SRC);
