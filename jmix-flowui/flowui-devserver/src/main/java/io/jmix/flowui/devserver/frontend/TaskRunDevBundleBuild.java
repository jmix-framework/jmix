...
public class TaskRunDevBundleBuild implements FallibleCommand {
    ...
    public static boolean needsBuild(Options options,
        ...
        try {
            boolean needsBuild = needsBuildInternal(options,
                    frontendDependencies, themeDefinition, finder);
            if (needsBuild) {
    ...
    protected static boolean needsBuildInternal(Options options,
        ...
        File npmFolder = options.getStudioFolder();
        ...
        if (!frontendImportsFound(statsJson, options, finder, themeDefinition, frontendDependencies)) {
            UsageStatistics.markAsUsed(
                    "flow/rebundle-reason-missing-frontend-import", null);
            return true;
        }

        if (themeConfigurationChanged(options, statsJson, themeDefinition)) {
            UsageStatistics.markAsUsed(
    ...
    private static boolean exportedWebComponents(JsonObject statsJson,
        ...
        try {
            ...
            exporterRelatedClasses.addAll(finder.getSubTypesOf(WebComponentExporter.class.getName()));
            exporterRelatedClasses.addAll(finder.getSubTypesOf(WebComponentExporterFactory.class.getName()));
    ...
    private static boolean frontendImportsFound(JsonObject statsJson,
                                                Options options, ClassFinder finder, ThemeDefinition themeDefinition,
                                                FrontendDependenciesScanner frontendDependencies)
            throws IOException {
        ...
        final GenerateMainImports generateMainImports = new GenerateMainImports(
                finder, frontendDependencies, options, themeDefinition, statsJson);
     ...
     *
     * @param frontendDirectory folder with frontend resources in the project
     * @param remainingImports  frontend resource imports listed in the bundle, but not found
     *                          with class scanner.
     * @return true if changes detected, false otherwise
     * @throws IOException if exception is thrown while reading files content
     */
     ...
     *
     * @param statsJson   devBundle statsJson
     * @param packageJson packageJson
     * @param npmPackages npm packages map
     ...
     *
     * @param options              the task options
     * @param frontendDependencies frontend dependency scanner
     * @param finder               classfinder
     * @return package.json content as JsonObject
     */
    private static JsonObject getPackageJson(Options options,
                                             FrontendDependenciesScanner frontendDependencies,
                                             ClassFinder finder) {
        File packageJsonFile = new File(options.getStudioFolder(), "package.json");
    ...
    protected static JsonObject getDefaultPackageJson(Options options,
        ...
        try {
            if (packageJson == null) {
                packageJson = nodeUpdater.getPackageJson(nodeUpdater.getStudioJsonFile());
            }
     ...
     *
     * @param npmPackages  {@code @NpmPackage} key-value map
     * @param dependencies json object containing dependencies to check against
     ...
     *
     * @param packageJson content of the package.json content red from a file
     */
