...
class VersionsJsonFilter {
    ...
    private JsonObject collectFrameworkVersions(JsonObject packageJson) {
        if (packageJson.hasKey(NodeUpdater.VAADIN_DEP_KEY) && packageJson
                .getObject(NodeUpdater.VAADIN_DEP_KEY).hasKey(dependenciesKey)) {
            return packageJson.getObject(NodeUpdater.VAADIN_DEP_KEY)
