...
For agent-driven usage, see [studio-meta-generator.skill](studio-meta-generator.skill) in this directory.
...
- `StudioPropertyGroupsMatcher` scans existing `@StudioPropertyGroup` declarations and adds compatible
  `propertyGroups` matches. Category-free group properties may be reused for categorized generated properties.
...
- `propertyGroups` are added only for compatible matches against existing groups.
- Known `xmlAttribute` names are rendered via `StudioXmlAttributes` constants instead of raw string literals.
...
The suite also keeps two migration guardrails in place:

- generated source should use `StudioXmlAttributes` for known XML attributes;
- production `@StudioUiKit` entries with more than three inline `properties` must declare at least one
  `propertyGroup`.
