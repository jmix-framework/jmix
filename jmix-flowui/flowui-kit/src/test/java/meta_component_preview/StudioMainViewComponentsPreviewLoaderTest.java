...
class StudioMainViewComponentsPreviewLoaderTest {
    ...
    @Test
    void testDoesNotSupportOtherNamespaces() {
        assertFalse(loader.isSupported(element("appLayout", Namespace.get("http://other"))));
        assertFalse(loader.isSupported(element("view", Namespace.get("http://other"))));
    }

    /**
     * Studio hangs the entire preview tree off the root's component, so a declined root renders
     * nothing at all — the roots must be claimed even though their runtime classes are Spring-managed.
     */
    @Test
    void testSupportsViewAndMainViewRoots() {
        assertTrue(loader.isSupported(element("view", VIEW_NS)));
        assertTrue(loader.isSupported(element("mainView", MAIN_VIEW_NS)));
    }

    /** Matches the stand-in the Studio meta declares: {@code VerticalLayout view()} / {@code mainView()}. */
    @Test
    void testLoadsRootsAsVerticalLayoutStandIn() {
        assertInstanceOf(VerticalLayout.class,
                loader.load(element("view", VIEW_NS), element("view", VIEW_NS)));
        assertInstanceOf(VerticalLayout.class,
                loader.load(element("mainView", MAIN_VIEW_NS), element("mainView", MAIN_VIEW_NS)));
    }
