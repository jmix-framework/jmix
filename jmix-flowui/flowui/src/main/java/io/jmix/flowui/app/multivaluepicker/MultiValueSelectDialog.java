...
public class MultiValueSelectDialog<E> extends StandardListView<E> implements MultiValueSelectView<E> {
    ...
    protected void initAddComponentLayout() {
        ...
        if (context.getJavaClass() != null
                // In this case JmixMultiSelectComboBoxPicker is generated by default
                // and it's inconvenient to add a value on a value change event
                || context.getEntityName() != null && !context.isUseComboBox()) {
    ...
    protected void addValueInternal(HasValue<?, E> component) {
        if (component instanceof EntityMultiPickerComponent) {
            Collection<E> items = UiComponentUtils.getValue((HasValue<?, Collection<E>>) component);
            addValueInternal(component, items);
        } else {
            E value = UiComponentUtils.getValue(component);
            if (value != null) {
                addValueInternal(component, Collections.singletonList(value));
            }
        }
    }

    @SuppressWarnings("unchecked")
    protected void addValueInternal(HasValue<?, E> component, Collection<E> itemsToAdd) {
        if (CollectionUtils.isNotEmpty(itemsToAdd)
                && (!(component instanceof SupportsValidation) || isValid((SupportsValidation<E>) component))) {
            component.clear();
            addValuesToLayout(itemsToAdd);
        }
    }
    ...
    protected void addValuesToLayout(Collection<E> items) {
        for (E item : items) {
            if (item != null && !valueExists(item)) {
                addValueToLayout(item);
            }
        }
    }
