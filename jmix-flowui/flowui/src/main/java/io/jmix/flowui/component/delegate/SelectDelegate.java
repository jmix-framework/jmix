...
@Component("flowui_SelectDelegate")
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class SelectDelegate<C extends JmixSelect<V>, T, V> extends AbstractFieldDelegate<C, T, V> {

    protected DataAwareComponentsTools dataAwareComponentsTools;

    public SelectDelegate(C component) {
        super(component);
    }
    ...
    protected void setupProperties(ValueSource<T> valueSource) {
        if (valueSource instanceof EntityValueSource<?, ?> entityValueSource) {
            dataAwareComponentsTools.setupEmptySelectionAllowed(component, entityValueSource);
        }
    }
}
