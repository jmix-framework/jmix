...
public class SideDialog extends JmixSideDialog implements ApplicationContextAware, InitializingBean {
     ...
     * <p>
     * The listener is informed when the user wants to close the dialog by clicking outside the dialog or by pressing
     * escape. Then you can decide whether to close or to keep opened the dialog. It means that dialog won't
     * be closed automatically unless you call {@link #close()} method explicitly in the listener implementation.
     * <p>
     * NOTE: adding this listener changes the behavior of the dialog. Dialog is closed automatically in case there
     * are no any close listeners. And the {@link #close()} method should be called explicitly to close the dialog
     * in case there are close listeners.
     *
     * @param listener listener to add
     * @return a registration for removing the listener
     */
    public Registration addDialogCloseActionListener(ComponentEventListener<SideDialogCloseActionEvent> listener) {
