...
public class RouteSupport {
    ...
    /**
     * Invokes {@code history.replaceState} in the browser with a new location
     * generated by combining the url which navigates to given navigation target
     * using given parameters and current query parameters.
     *
     * @param ui               UI instance for which to update url
     * @param navigationTarget navigation target
     * @param routeParameters  route parameters
     * @throws NotFoundException in case the navigationTarget is not registered with
     *                           a url template matching the given parameters.
     */
    public void replaceUrl(UI ui,
        ...
        page.fetchCurrentURL(url -> {
            ...
            String locationString = getRouteConfiguration().getUrl(navigationTarget, routeParameters);
