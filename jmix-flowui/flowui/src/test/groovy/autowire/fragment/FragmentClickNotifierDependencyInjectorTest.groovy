...
@SpringBootTest
class FragmentClickNotifierDependencyInjectorTest extends FlowuiTestSpecification {
    ...
    def "Autowire #clickEvent into the fragment"() {
        def hostView = navigateToView AutowireFragmentHostView

        when: "Fragment component is created"
        def fragment = fragments.create hostView, ClickNotifierDependencyInjectorFragment

        then: "#clickEvent listener should be assigned to the corresponding button"
        def currentButton = fragment."${clickEvent}Id" as JmixButton
        currentButton != null
        currentButton.click()
