...
@SpringBootTest
class EntityComboBoxValueChangeTest extends FlowuiTestSpecification {
    ...
    @Override
    void setup() {
        registerViewBasePackages("component.entitycombobox.view")
        setupData()
    }
    ...
    def "value change events via setValueFromClient must be marked as from client"() {
        given: "An EntityComboBox opened in a detail view"
        navigateToView(EntityComboBoxOrderLineListTestView)
        def orderLineListView = UiTestUtils.currentView as EntityComboBoxOrderLineListTestView
        orderLineListView.createButton.click()
        def orderLineDetailView = UiTestUtils.currentView as EntityComboBoxOrderLineDetailTestView
        ...
        def fromClientFlags = []
        orderLineDetailView.productField.addValueChangeListener({ event -> fromClientFlags << event.fromClient })

        when: "User selects a value (LookupSelectAction) and then clears it (EntityClearAction)"
        orderLineDetailView.productField.setValueFromClient(product)
