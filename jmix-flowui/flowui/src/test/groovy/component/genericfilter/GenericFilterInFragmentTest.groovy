...
class GenericFilterInFragmentTest extends FlowuiTestSpecification {
    ...
    protected static PropertyFilter<?> addCondition(GenericFilter filter, String property) {
        PropertyFilter<?> condition = filter.filterComponentBuilder()
                .propertyFilter()
                .property(property)
                .operation(PropertyFilter.Operation.EQUAL)
                .build()

        return addToCurrentConfiguration(filter, condition)
    }

    protected static GroupFilter addGroupCondition(GenericFilter filter) {
        PropertyFilter<?> nested = filter.filterComponentBuilder()
                .propertyFilter()
                .property("number")
                .operation(PropertyFilter.Operation.EQUAL)
                .build()
     ...
     */
    protected static <T> T addToCurrentConfiguration(GenericFilter filter, T condition) {
        def configuration = filter.currentConfiguration
