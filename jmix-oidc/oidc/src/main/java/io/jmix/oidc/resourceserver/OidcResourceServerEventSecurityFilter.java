 ...
 */
public class OidcResourceServerEventSecurityFilter extends OncePerRequestFilter {

    private static final Logger log = LoggerFactory.getLogger(OidcResourceServerEventSecurityFilter.class);

    protected ApplicationEventPublisher applicationEventPublisher;

    public OidcResourceServerEventSecurityFilter(ApplicationEventPublisher applicationEventPublisher) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (applicationEventPublisher != null && authentication != null) {
                OidcResourceServerBeforeInvocationEvent beforeInvocationEvent = new OidcResourceServerBeforeInvocationEvent(authentication, request, response);
                ...
                try {
                    if (!invocationPrevented) {
                        ...
                        log.debug("Request invocation prevented by BeforeOidcResourceServerApiInvocationEvent handler");
                    ...
                    applicationEventPublisher.publishEvent(new OidcResourceServerAfterInvocationEvent(authentication, request, response, invocationPrevented));
                }
            } else {
                filterChain.doFilter(request, response);
            }
