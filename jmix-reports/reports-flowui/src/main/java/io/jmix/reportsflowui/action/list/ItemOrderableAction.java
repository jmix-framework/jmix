...
public class ItemOrderableAction<E extends OrderableEntity> extends SecuredListDataComponentAction {
    ...
    public void execute() {
        ...
        int selectedCnt = target.getSelectedItems().size();
        if (selectedCnt == 1) {
    ...
    protected void swapSingleItemWithNeighbour() {
        OrderableEntity selectedItem = (OrderableEntity) target.getSingleSelectedItem();
        ...
        for (ListIterator<OrderableEntity> iterator = allItems.listIterator(); iterator.hasNext(); ) {
            if (iterator.next().equals(target.getSingleSelectedItem())) {
    ...
    protected void moveFewItems() {
        ...
        Set<E> selectedItems = target.getSelectedItems();
        int spreadKoef = target.getSelectedItems().size();//U can use 10 for easier debug
