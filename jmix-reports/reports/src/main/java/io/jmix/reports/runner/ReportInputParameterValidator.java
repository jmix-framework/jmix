...
/**
 * Validates report input parameters before running the report. Used by any code that runs a report on behalf of
 * an external caller (e.g. UI or REST API) to check parameter values the same way, so a report is not executed with
 * invalid input.
 */
@Component("report_ReportInputParameterValidator")
public class ReportInputParameterValidator {
    private static final Logger log = LoggerFactory.getLogger(ReportInputParameterValidator.class);
    ...
    public ReportInputParameterValidator(GroovyScriptParametersProvider groovyScriptParametersProvider,
                                         Scripting scripting,
    ...
    protected void runValidationScript(String groovyScript, Map<String, Object> scriptContext) {
        if (StringUtils.isNotBlank(groovyScript)) {
            ...
            wrapValidation(() -> scripting.evaluateGroovy(groovyScript, scriptContext));
        }
    }
    ...
    protected void runCrossValidationDelegate(ParametersCrossValidator crossValidator, Map<String, Object> reportParameters) {
        wrapValidation(() -> crossValidator.validateParameters(reportParameters));
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    protected void runValidationDelegate(ParameterValidator validator, Object value) {
        wrapValidation(() -> validator.validate(value));
    }
