...
/**
 * Engine-independent contract for rendering a single {@code ${alias}} placeholder: the Excel cell type and
 * value produced for every supported Java type, and the effect of a value format.
 *
 * <p>Both engines must satisfy it, so every scenario here runs twice — once through
 * {@link XlsxCellValueTest} (the in-memory engine) and once through
 * {@code StreamingXlsxCellValueTest} (the streaming engine, which only swaps
 * {@link BaseXlsxRenderTest#createFormatter}). Typing parity between the two engines is a documented promise
 * of the streaming formatter, so it is verified by running the same scenarios rather than by maintaining a
 * hand-written copy per engine — copies drift, and a drifting copy asserts parity it no longer measures.
 *
 * <p>A scenario that must legitimately behave differently per engine does NOT belong here: keep it in the
 * concrete spec of the engine it applies to, so the divergence is explicit and reviewable.
 */
abstract class BaseXlsxCellValueTest extends BaseXlsxRenderTest {
    ...
    def "a numeric literal in a band cell keeps its type in every rendered row"() {
        given: 'a template cell holding a number, not a ${alias} string'
            def template = buildTemplate { wb ->
                def sheet = sheet(wb)
                numericCell(sheet, 0, 0, 7.5d)
                cell(sheet, 0, 1, '${name}')
                defineBand(wb, "Data", 0, 0, 0, 1)
            }
            def root = rootBand("Data")
            addBand(root, "Data", [name: "a"])
            addBand(root, "Data", [name: "b"])

        when:
            def sheet = renderAndReadFirstSheet(template, root)

        then: "the literal is copied as a number to each instance, not stringified"
            requireCell(sheet, 0, 0).cellType == CellType.NUMERIC
            numericValue(sheet, 0, 0) == 7.5d
            requireCell(sheet, 1, 0).cellType == CellType.NUMERIC
            numericValue(sheet, 1, 0) == 7.5d
    }

    def "a boolean literal in a band cell keeps its type in every rendered row"() {
        given:
            def template = buildTemplate { wb ->
                def sheet = sheet(wb)
                booleanCell(sheet, 0, 0, true)
                cell(sheet, 0, 1, '${name}')
                defineBand(wb, "Data", 0, 0, 0, 1)
            }
            def root = rootBand("Data")
        ...
        then:
            requireCell(sheet, 0, 0).cellType == CellType.BOOLEAN
            booleanValue(sheet, 0, 0)
            requireCell(sheet, 1, 0).cellType == CellType.BOOLEAN
            booleanValue(sheet, 1, 0)
    }
