...
public class RoleCodeUniquenessValidator implements SmartInitializingSingleton {
    ...
    public void validate() {
        checkTypeUniqueness(resourceRoleRepository.getAllRoles(), "resource");
        checkTypeUniqueness(rowLevelRoleRepository.getAllRoles(), "row-level");
    }
        ...
        for (Map.Entry<String, List<BaseRole>> entry : rolesByCode.entrySet()) {
            ...
            if (sameCodeRoles.size() > 1 && hasAnnotated) {
                throw new DuplicateRoleCodeException(
                        String.format("Duplicate %s role code '%s': a design-time (annotated) role conflicts " +
                                "with a runtime (database) role with the same code", roleType, entry.getKey()),
                        entry.getKey());
            }
        }
    }
