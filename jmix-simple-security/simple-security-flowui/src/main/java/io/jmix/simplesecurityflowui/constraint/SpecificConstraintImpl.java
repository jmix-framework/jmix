...
public class SpecificConstraintImpl implements SpecificConstraint<SpecificOperationAccessContext> {
    ...
    public SpecificConstraintImpl(CurrentAuthentication currentAuthentication) {
    ...
    public void applyTo(SpecificOperationAccessContext context) {
        boolean authorityFound = currentAuthentication.getUser().getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .anyMatch(authority -> authority.equals(context.getName()));
        if (!authorityFound) {
            context.setDenied();
        }
    }
