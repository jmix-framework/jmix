...
import ${updateService.getQualifiedName()}<%}%>
...
class ${viewControllerName}<%if (useDataRepositories || useUpdateServiceInController){%>(<%if (useDataRepositories){%>private val repository: ${repository.getName()}<%}%><%if (useDataRepositories && useUpdateServiceInController){%>, <%}%><%if (useUpdateServiceInController){%>private val updateService: ${updateService.getName()}<%}%>)<%}%> : StandardListView<${entity.className}>() {
    ...
    private fun listLoadDelegate(pageable: Pageable, context: JmixDataRepositoryContext): List<${entity.className}> {
...
<%if (useUpdateServiceSaveDelegate){%>
    ...
    private fun saveDelegate(saveContext: SaveContext): Set<Any> {
        ...
        if (!compositeAttrs.isEmpty()) {
            out.println("        // ${entity.className} has the following @Composition attributes: ${compositeAttrs.join(', ')}.")
            out.println("        // Make sure they have CascadeType.ALL in @OneToMany annotation.")
        }%>return mutableSetOf(updateService.save(${detailDc}.item, saveContext))
    }<%}%>
}