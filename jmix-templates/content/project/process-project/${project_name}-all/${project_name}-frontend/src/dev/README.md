...
React Buddy palettes describe reusable formComponents and building blocks. `React Palette` tool window becomes available
when an editor with React formComponents is active. You can drag and drop items from the tool window to the code editor or
JSX Outline. Alternatively, you can insert formComponents from the palette using code generation
action (`alt+insert` / `⌘ N`).

Add formComponents to the palette using `Add to React Palette` intention or via palette editor (look for the corresponding
