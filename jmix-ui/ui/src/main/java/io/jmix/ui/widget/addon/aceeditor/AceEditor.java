...
//		@Override
	...
	private AceEditorServerRpc rpc = new AceEditorServerRpc() {
		@Override
		public void changed(TransportDiff diff, TransportDoc.TransportRange selection,
                            boolean focused) {
		...
		public void changedDelayed(TransportDiff diff,
                                   TransportDoc.TransportRange selection, boolean focused) {
			clientChanged(diff, selection, focused);
		}
	};
	 ...
	 * @return marker id
	 */
	public String addMarker(AceRange range, String cssClass, AceMarker.Type type,
			boolean inFront, AceMarker.OnTextChange onChange) {
	...
	public void addMarkerAnnotation(AceAnnotation ann, String markerId) {
		doc = doc.withAdditionalMarkerAnnotation(new AceAnnotation.MarkerAnnotation(markerId,
				ann));
		markAsDirty();
	}

	public void addRowAnnotation(AceAnnotation ann, int row) {
		doc = doc.withAdditionalRowAnnotation(new AceAnnotation.RowAnnotation(row, ann));
		markAsDirty();
	}
	...
	public void beforeClientResponse(boolean initial) {
		...
		if (selectionToClient != null) {
			...
			// {startRow,startCol,endRow,endCol}
				...
				TransportDoc.TransportRange tr = new TransportDoc.TransportRange(selectionToClient[0],
	...
	public void clearMarkerAnnotations() {
		Set<AceAnnotation.MarkerAnnotation> manns = Collections.emptySet();
	...
	public void clearRowAnnotations() {
		Set<AceAnnotation.RowAnnotation> ranns = Collections.emptySet();
...
//	}
	...
	protected void clientChanged(TransportDiff diff, TransportDoc.TransportRange selection,
			boolean focused) {
		diffFromClient(diff);
	...
	private void selectionFromClient(TransportDoc.TransportRange sel) {
