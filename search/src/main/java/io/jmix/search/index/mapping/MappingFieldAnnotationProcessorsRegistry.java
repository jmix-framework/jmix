...
public class MappingFieldAnnotationProcessorsRegistry {
    ...
    @Autowired
    public MappingFieldAnnotationProcessorsRegistry(List<FieldAnnotationProcessor<?>> mappingFieldDescriptorFactories) {
        Map<Class<? extends Annotation>, FieldAnnotationProcessor<?>> tmpRegistry = new HashMap<>();
        for(FieldAnnotationProcessor<?> factory : mappingFieldDescriptorFactories) {
            log.info("[IVGA] Register processor '{}' for annotation '{}'", factory, factory.getAnnotationClass());
            tmpRegistry.put(factory.getAnnotationClass(), factory);
        }
