...
package io.jmix.ui.widget.client.calendar.schedule;
...
import io.jmix.ui.widget.client.calendar.JmixCalendarWidget;
...
public class JmixDateCell extends DateCell {
    ...
    public JmixDateCell(WeekGrid parent, Date date) {
        super(parent, date);
        ...
        if (event.getTypeInt() == Event.ONCLICK && !rangeSelect) {
            ...
            for (DateCellSlot slot : slots) {
                ...
                if (containsSlotNumber(className, targetSlotNumber)) {
                    JmixCalendarWidget calendar = (JmixCalendarWidget) weekgrid.getCalendar();
