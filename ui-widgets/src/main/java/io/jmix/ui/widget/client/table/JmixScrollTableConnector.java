...
import io.jmix.ui.widget.client.tableshared.JmixTableShortcutActionHandler;
import io.jmix.ui.widget.client.tableshared.TableCellClickListener;
...
import static io.jmix.ui.widget.client.Tools.findCurrentOrParentTd;

@Connect(value = JmixTable.class, loadStyle = Connect.LoadStyle.EAGER)
public class JmixScrollTableConnector extends TableConnector {

    protected static final String HAS_FOOTER_STYLENAME = "has-footer";

    protected HandlerRegistration tooltipHandlerRegistration;

    public JmixScrollTableConnector() {
        registerRpc(JmixTableClientRpc.class, new JmixTableClientRpc() {
            @Override
    ...
    @Override
    public JmixTableState getState() {
        return (JmixTableState) super.getState();
    }

    @Override
    public JmixScrollTableWidget getWidget() {
        return (JmixScrollTableWidget) super.getWidget();
    }
    ...
    public TooltipInfo getTooltipInfo(Element element) {
        ...
        if (element != getWidget().getElement()) {
            Object node = WidgetUtil.findWidget(element, JmixScrollTableWidget.JmixScrollTableBody.JmixScrollTableRow.class);

            if (node != null) {
                JmixScrollTableWidget.JmixScrollTableBody.JmixScrollTableRow row
                        = (JmixScrollTableWidget.JmixScrollTableBody.JmixScrollTableRow) node;
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                if (childUidl.getTag().equals("shortcuts")) {
                    if (getWidget().getShortcutActionHandler() == null) {
                        getWidget().setShortcutActionHandler(new JmixTableShortcutActionHandler(uidl.getId(), client, this));
                    }
    ...
    protected void init() {
        ...
        getWidget()._delegate.cellClickListener = new TableCellClickListener() {
            ...
            public void onClick(String columnKey, int rowKey, boolean isText) {
                getRpcProxy(JmixTableServerRpc.class).onClick(columnKey, String.valueOf(rowKey), isText);
            }
        };
        ...
        getWidget()._delegate.totalAggregationInputHandler = (columnKey, value, isFocused) -> {
            getRpcProxy(JmixTableServerRpc.class).onAggregationTotalInputChange(columnKey, value, isFocused);
        };

        getWidget()._delegate.emptyStateLinkClickHandler = () -> getRpcProxy(JmixTableServerRpc.class).onEmptyStateLinkClick();
    }
