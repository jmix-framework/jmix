...
import io.jmix.ui.widgets.client.textfield.JmixMaskedFieldWidget;
...
public class JmixDateFieldWidget extends VPopupCalendar implements ShortcutActionHandler.ShortcutActionHandlerOwner {
    ...
    public JmixMaskedFieldWidget getImpl() {
        return (JmixMaskedFieldWidget) super.getImpl();
    }

    @Override
    protected JmixMaskedFieldWidget createImpl() {

        return new JmixMaskedFieldWidget() {
                ...
                if (newText != null
                    ...
                    if (validateText(newText)) {
                        if (!newText.equals(nullRepresentation)) {
                            getElement().removeClassName(JmixDateFieldWidget.EMPTY_FIELD_CLASS);
                        }
                        JmixDateFieldWidget.this.onChange(null);
